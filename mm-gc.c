 ...
 */
static int is_pointer(void * ptr){
  if (ptr == NULL || ((unsigned) ptr & 0x7) != 0)
    return 0;
  ...
  while(curBlock < heapFooter && UNSCALED_POINTER_ADD(curBlock, WORD_SIZE) <= ptr){
 ...
 */
static void mark(void * ptr){
  size_t *blockHeader = (size_t *) UNSCALED_POINTER_SUB(ptr, WORD_SIZE), i, n;

  if ((*blockHeader)&BLOCK_MARKED)
    return;

  *blockHeader |= BLOCK_MARKED;
  n = SIZE(*blockHeader) - WORD_SIZE;
  for (i = 0; i < n; i += WORD_SIZE) {
    void **p = (void**) UNSCALED_POINTER_ADD(ptr, i);
    if (is_pointer(*p)) {
      blockHeader = (size_t *) UNSCALED_POINTER_SUB(*p, WORD_SIZE);
      if (!((*blockHeader)&BLOCK_MARKED))
        mark(*p);
    }
  }
...
static void sweep(){
  ...
  while (curBlock < heapFooter) {
    size_t allocated = *curBlock & TAG_USED;
    size_t marked = *curBlock & BLOCK_MARKED;

    if (!marked && allocated){
      mm_free(UNSCALED_POINTER_ADD(curBlock, WORD_SIZE));
    }

    *curBlock &= (~BLOCK_MARKED);
    curBlock = (size_t *) UNSCALED_POINTER_ADD(curBlock, SIZE(*curBlock));
  }
}
