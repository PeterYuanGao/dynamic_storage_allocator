...
static void requestMoreSpace(size_t reqSize) {
  ...
  /* initialize header, inherit TAG_PRECEDING_USED status from the
     ...
     bit */
  newBlock->sizeAndTags &= TAG_PRECEDING_USED;
  newBlock->sizeAndTags |= totalSize; 
...
static void setBoundaryTag(BlockInfo* ptr) {
  *((size_t*)UNSCALED_POINTER_ADD(ptr, SIZE(ptr->sizeAndTags)-WORD_SIZE)) = SIZE(ptr->sizeAndTags);
}
...
void* mm_malloc(size_t size) {
  ...
  if (size <= MIN_BLOCK_SIZE) {
    ...
    // (we'll need to use the next pointer, the prev pointer, and the boundary tag).
    reqSize = MIN_BLOCK_SIZE;
  } else {
    // Round up for correct alignment
    reqSize = ALIGNMENT * ((size + ALIGNMENT - 1) / ALIGNMENT);
  }
  // Find a free block of at least reqSize.
  ptrFreeBlock = (BlockInfo*)searchFreeList(reqSize);
  if (ptrFreeBlock == NULL) 
    requestMoreSpace(reqSize), ptrFreeBlock = (BlockInfo*)searchFreeList(reqSize);
  // Initialize the allocated block.
  ptrFreeBlock->sizeAndTags |= TAG_USED, removeFreeBlock(ptrFreeBlock);
  blockSize = SIZE(ptrFreeBlock->sizeAndTags);
  BlockInfo* next = (BlockInfo*) UNSCALED_POINTER_ADD(ptrFreeBlock, blockSize);
  // Splitting the free block if necessary.
  if (blockSize >= reqSize + MIN_BLOCK_SIZE) {
    BlockInfo* splitted = (BlockInfo*) UNSCALED_POINTER_ADD(ptrFreeBlock, reqSize);
    splitted->sizeAndTags = (blockSize - reqSize) | TAG_PRECEDING_USED;
    setBoundaryTag(splitted);
    insertFreeBlock(splitted);
    ptrFreeBlock->sizeAndTags = reqSize | (ptrFreeBlock->sizeAndTags&0x3);
  } else if (next < (BlockInfo*) mem_heap_hi()) {
    next->sizeAndTags |= TAG_PRECEDING_USED;
    if ((next->sizeAndTags & TAG_USED) == 0) 
      setBoundaryTag(next);
  }
